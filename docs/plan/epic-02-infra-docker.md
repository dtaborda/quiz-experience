...
## Tasks
- [ ] Create `infra/docker-compose.yml` with frontend + backend services (shared network, env vars, volumes for hot reload).
- [ ] Author `frontend.Dockerfile` (multi-stage: deps, builder, runtime) tailored to Next.js 15.
- [ ] Author `backend.Dockerfile` (multi-stage) with production-ready Express server.
- [ ] Add `.env.example` covering all required environment variables.
- [ ] Document usage (`pnpm dev`, `docker compose up`, Devbox instructions) in `docs/infra.md` or README.
