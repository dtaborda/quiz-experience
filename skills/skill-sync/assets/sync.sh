...
# Collect skills by scope (Bash 3 compatible: use temp file instead of associative array)
SCOPE_DATA=$(mktemp)
trap 'rm -f "$SCOPE_DATA"' EXIT
...
while IFS= read -r skill_file; do
    ...
    # But we also use '|' to separate entries, so we protect it.
    ...
    for scope in "${scopes[@]}"; do
        ...
        # Store: scope<TAB>skill_name:auto_invoke
        echo -e "$scope\t$skill_name:$auto_invoke" >> "$SCOPE_DATA"
    done
...
|--------|-------|"
    ...
    grep "^$scope"$'\t' "$SCOPE_DATA" | while IFS=$'\t' read -r _ entry; do
        skill_name="${entry%%:*}"
        actions_raw="${entry#*:}"
...
| $action | \`$skill_name\` |"
    done < <(LC_ALL=C sort -t $'\t' -k1,1 -k2,2 "$rows")
